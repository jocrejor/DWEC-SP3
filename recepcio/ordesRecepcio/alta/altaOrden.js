...
function iniciar() {
    ...
    document.getElementById("btnGravar").addEventListener("click", gravarOrden, false);
    document.getElementById("btnGuardar").addEventListener("click", guardarModificacion, false);
    document.getElementById("llistatOrden").addEventListener("click", llistarOrden, false);
}
...
function afegirLinea(productObj) {
    ...
    linea.appendChild(esborrarTD);productID
...
function gravarOrden() {
    ...
    if(orderReception.length == 0) {
        ...
        const maxObj = orderRception.reduce((max, obj) => (obj.id > max.id ? obj : max), orderReception[0]);
...
function validar(e) {
    ...
    if (validarProveidor() && validarDataEstima() && validarProducto() && validarCantidadPedida()) {
        afegirProducte();
        return true;
